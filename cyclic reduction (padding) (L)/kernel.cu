...
* time is measured
* padding is done to avoid divergence
...
__global__ void cr_forward(float *a, float *b, float *c, float *f, float *x,int size,int padding,int step_size,int i){
...
__global__ void cr_backward(float *a, float *b, float *c, float *f, float *x,int size,int padding,int step_size,int i){
	int index_x = blockIdx.x * blockDim.x + threadIdx.x;
	int index_y = blockIdx.y * blockDim.y + threadIdx.y;
	int grid_width = gridDim.x * blockDim.x;
	int threadid = index_y * grid_width + index_x;
...
void init_data(float **a,float **b,float **c,float **f,float **x,int size){
	int i,j;
	int padding = 2*log2(size+1);
	*a = (float *) malloc(sizeof(float)*(size+padding));
	*b = (float *) malloc(sizeof(float)*(size+padding));
	*c = (float *) malloc(sizeof(float)*(size+padding));
	*f = (float *) malloc(sizeof(float)*(size+padding));
	*x = (float *) malloc(sizeof(float)*(size+padding));
	...
	for(i=log2(size+1);i<size+log2(size+1);i++){
		(*a)[i] = -1.0;
		(*b)[i] =  4.0;
		(*c)[i] = -1.0;
		(*f)[i] = 15.0;
	...
	for(i=size+log2(size+1);i<size+padding;i++){
		(*a)[i] = 0.0;
		(*b)[i] = 0.0;
		(*c)[i] = 0.0;
		(*x)[i] = 0.0;
...
//calculate dimension of the block according to size
...
int main(){
	...
	cudaMalloc(&d_a,sizeof(float)*(size+padding));
	cudaMalloc(&d_b,sizeof(float)*(size+padding));
	cudaMalloc(&d_c,sizeof(float)*(size+padding));
	cudaMalloc(&d_x,sizeof(float)*(size+padding));
	cudaMalloc(&d_f,sizeof(float)*(size+padding));
	...
	start = clock();
	//copy to device
	cudaMemcpy(d_a,a,sizeof(float)*(size+padding),cudaMemcpyHostToDevice);
	cudaMemcpy(d_b,b,sizeof(float)*(size+padding),cudaMemcpyHostToDevice);
	cudaMemcpy(d_c,c,sizeof(float)*(size+padding),cudaMemcpyHostToDevice);
	cudaMemcpy(d_f,f,sizeof(float)*(size+padding),cudaMemcpyHostToDevice);
	cudaMemcpy(d_x,x,sizeof(float)*(size+padding),cudaMemcpyHostToDevice);
	...
	for(i=0;i<log2(size+1)-1;i++){
		...
		cr_forward<<<dimGrid,dimBlock>>>(d_a, d_b, d_c, d_f, d_x,size,padding/2,step_size,i);
	}
	...
	cr_div<<<1,1>>>(d_b,d_f,d_x,padding/2+(size-1)/2);
	cudaDeviceSynchronize();
	...
	for(i=log2(size+1)-2;i>=0;i--){
		step_size = (size-pow(2.0,i+1))/pow(2.0,i+1)+1;
		calc_dim(step_size,&dimBlock,&dimGrid);
		cr_backward<<<dimGrid,dimBlock>>>(d_a, d_b, d_c, d_f, d_x,size,padding/2,step_size,i);
	}
	if(error != cudaSuccess)  {    
	...
	//copy back to host
	cudaMemcpy(x,d_x,sizeof(float)*(size+padding),cudaMemcpyDeviceToHost);
	cudaDeviceSynchronize();
	stop = clock();
	...
	cpu_cr(&a[padding/2],&b[padding/2],&c[padding/2],&f[padding/2],x_cpu,size);
	//check for correctness
    diff_results(&x[padding/2],x_cpu,size);
